...
class SiteController extends Controller
{
    ...
    {
        if (isset($id)){
            $product = Product::find()->where(['=', 'categore_id', $id])->all();
        }else{
    ...
    public function actionCategore($id)
    {
        $product = Product::find()->where(['=', 'id', $id])->one();
        return $this->render('categore', ['product'=>$product]);
    }
    ...
    public function actionClent()
    {
        ...
        $id = $post['id'];
        if (isset($post)){
            $clent->clent_name = $post['name'];
            $clent->clent_list_name = $post['famle'];
            $clent->product_tell = $post['tell'];
            $clent->save();
            Yii::$app->session->setFlash('success',  'Ваш запрос был успешно отправлен!');
        }
        return $this->redirect(['site/categore', 'id'=>$id]);
    }
